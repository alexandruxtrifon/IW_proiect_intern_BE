	...
	IF (@Email IS NULL AND @NrTel IS NULL)
        ...
        BEGIN;
            ...
            BEGIN
                ...
                IF LEN(@NrTele) != 10
                BEGIN
                    SET @Message = 'Numar de telefon prea lung/scurt: ' + @NrTele;
...
GO
CREATE OR ALTER PROCEDURE actualizareStatusClient
	@Cod_Client INT,
	@Activ BIT
AS
BEGIN
    ...
    BEGIN TRY
		...
		IF EXISTS(SELECT 1 FROM Clienti WHERE Cod_Client = @Cod_Client AND Activ = @Activ)
		BEGIN;
			IF @Activ = 1
			BEGIN;
			THROW 50001, 'Clientul este deja activv', 1;
			END
			ELSE IF @Activ = 0
			BEGIN;
			THROW 50002, 'Clientul este deja inactivv', 1;
			END
		END
